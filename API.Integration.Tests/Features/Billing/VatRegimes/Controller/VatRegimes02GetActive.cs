...
namespace VatRegimes {
    ...
    [Collection("Sequence")]
    public class Codes02GetActive : IClassFixture<AppSettingsFixture> {
        ...
        private readonly string _url = "/vatRegimes/getAutoComplete";

        #endregion

        public Codes02GetActive(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<CodeAutoCompleteVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
