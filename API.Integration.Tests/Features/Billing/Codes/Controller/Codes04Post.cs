...
namespace Codes {
    ...
    public class Codes04Post : IClassFixture<AppSettingsFixture> {
        ...
        [Theory]
        [ClassData(typeof(CreateValidCode))]
        ...
        [Theory]
        [ClassData(typeof(CreateValidCode))]
        public async Task Simple_Users_Can_Not_Create(TestCode record) {
            await Forbidden.Action(_httpClient, _baseUrl, _url, _actionVerb, "simpleuser", "1234567890", record);
        }

        [Theory]
        [ClassData(typeof(CreateValidCode))]
        public async Task Admins_Can_Create_When_Valid(TestCode record) {
