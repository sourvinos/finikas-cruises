...
namespace TaxOffices {
    ...
    [Collection("Sequence")]
    public class TaxOffices04Post : IClassFixture<AppSettingsFixture> {
        ...
        public TaxOffices04Post(AppSettingsFixture appsettings) {
        ...
        [Theory]
        [ClassData(typeof(CreateValidTaxOffice))]
        ...
        [Theory]
        [ClassData(typeof(CreateValidTaxOffice))]
        public async Task Simple_Users_Can_Not_Create(TestTaxOffice record) {
            await Forbidden.Action(_httpClient, _baseUrl, _url, _actionVerb, "simpleuser", "1234567890", record);
        }

        [Theory]
        [ClassData(typeof(CreateValidTaxOffice))]
        public async Task Admins_Can_Create_When_Valid(TestTaxOffice record) {
