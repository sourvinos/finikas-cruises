...
namespace TaxOffices {
    ...
    [Collection("Sequence")]
    public class TaxOffices02GetActive : IClassFixture<AppSettingsFixture> {
        ...
        private readonly string _url = "/taxOffices/getAutoComplete";

        #endregion

        public TaxOffices02GetActive(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<TaxOfficeAutoCompleteVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
            Assert.Equal(145, records.Count);
        }
