...
namespace PaymentMethods {
    ...
    [Collection("Sequence")]
    public class PaymentMethods02GetActive : IClassFixture<AppSettingsFixture> {
        ...
        private readonly string _url = "/paymentMethods/getAutoComplete";

        #endregion

        public PaymentMethods02GetActive(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<PaymentMethodAutoCompleteVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
